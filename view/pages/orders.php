                    ...
                    <tbody>
                        <?php foreach ($orders as $index => $order) { ?>
                            <tr>
                                ...
                                <td class="align-middle">
                                    <?php echo $order['user_payment_id']; ?>
                                </td>
                                <td class="align-middle">
                                ...
                                <td class="align-middle">
                                    ...
                                    <?php } elseif ($order['status'] == 'delivered') { ?>
                                        Delivered at <?php echo $order['order_time']; ?>
                                    <?php } ?>
                                </td>
                            </tr>
                        <?php } ?>
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $action = isset($_POST["accept_order"]) ? 'accept' : (isset($_POST["cancel_order"]) ? 'cancel' : (isset($_POST["deliver_order"]) ? 'deliver' : ''));
    $delete_order = isset($_POST["delete_order"]);

    if (!empty($action)) {
        if ($action == 'accept') {
            $updateResult = $orderProcessor->updateOrderStatus($order_id, $action);

            if ($updateResult) {
                // Redirect to the same page to display updated orders
                header("Location: " . $_SERVER['PHP_SELF']);
            } else {
                echo "Error updating order status: " . mysqli_error($connect);
            }
        } elseif ($action == 'cancel') {
            $deleteOrderResult = $orderProcessor->deleteProduct($order_id);
